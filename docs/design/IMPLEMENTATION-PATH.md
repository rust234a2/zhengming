...
| 1-4 | 终局评估走真 Host | 0-2 | ☐ |
| 1-5 | 选边制撮合落地 | 1-1…1-4 | ☐ |
| 2-1 | 树 schema 可持久化 | 1-5 | 🟡 种子与类型层已落地（`types/debateTree.ts` + `data/debateTreeSeed.ts`）；文档层 / repository / reducer 未做 |
| 2-2 | Agent 追问点走真 Host | 2-1 | ☐ |
| ~~2-3~~ | ~~辩论间终局上树（打通）~~ **已于 2026-09-13 作废**：辩论间 v0.4 去树化，不再产出 `settlement` | — | ✗ |
| 2-4 | 知乎冷启动建树 | 2-1 | 🟡 数据侧完成（见 2026-09-14 进展）：122 个真实议题 + 36 条真实论点入树；Host 两步法未走（复用已有抽取结果），持久化仍缺 |
...
**八个决策点（D1–D7 于 2026-09-13、D8 于 2026-09-14 拍板；AI 不许自己决定）**
...
| **D8** | ~~争议地图视觉增强（凸包 / 连线分层 / 标签密度）是否进主实施路径~~ **✅ 已拍板（2026-09-14）：demo 前必须做（进主表）** | P0 三条立为**卡 3-4**（浏览层支线，不阻塞主链）；PRD 升 v0.2（§5 视觉规格重写 + §3.4 数据口径修正）、新建 PLAN v0.1。P1/P2（分层边捆绑 / 聚焦透镜 / 密度等值线 / 小倍数等）**另立卡不并入本卡**；其中聚焦透镜属交互语义变更（"换图"→"留背景淡化"），动工前须过 PRD §4 层拍板。红线约束全程有效：凸包分组口径来自 bridge 边（测试锁定），31/106 议题不落包络属真实信息须如实呈现 |
...
**为什么地图最成熟却排第三**：它是**浏览层/放大器，不是发动机**——不产生新数据、不连接人。联动需要树里先有真实数据，否则点进去看到的是 mock。但地图有个被低估的价值：**它是冷启动数据源**（106 个真实议题 + 244 条真实论点可直接喂给树，解决"空树"）。
...
> **2026-09-14 进展（数据侧完成，卡未完成）**
>
> 落地了 `research/debate-tree/gen-tree-seeds.mjs` → `web/src/data/debateTreeSeed.ts`，`?view=debate` 已从内嵌 mock 换成真实语料。结果：**122 个真实议题**（27 个带真实论点共 36 条 / 3 个带真实回答摘要共 54 条 / 92 个仅题干）。
>
> **与原流程的三处偏差，都是有意为之：**
> 1. **不新采集**：不调 `search zhihu`。语料已在盘上（`controversy-map/claims.json` 的论点抽取、`zhihu-corpus/` 的真实回答与立场聚类），重复采集只烧额度。
> 2. **不重复做立场抽取**：原计划的「LLM 两步法」就是 `controversy-map` 管线里已经跑过的那一步，直接复用其结果，不二次调用模型——也顺带满足「模拟与事实分离」（生成物里没有任何模型生成内容）。
> 3. **产物名改为 `debateTreeSeed.ts`**（原计划 `debateTreeImport.ts`）：它的角色是**冷启动种子**而不是任意导入通道；「导入」应当是后面 `importSettlement`/手动导入的职责，两者不该共用一个入口。
>
> **两条新立的规则（后续所有语料接入都适用）：**
> - **知乎赞同数 ≠ 争鸣平台投票**：种子节点平台票一律从 0 起，真实赞同数只出现在 `source.voteUp`，界面上是另一枚独立标签。把 204 赞写进 `votes.up` 会让「三派占比」看起来有数据，而那些票并不存在。
> - **禁用词按字面扫描，所以真实原文节选也要过表**：有 6 条论点的真实回答正文里含「错误 / 对错 / 赢了」这类日常用词。处理方式是**省略该条原文节选**，保留论点、答主、赞同数与链接——provenance 不因省略而丢。生成器会把它计入剔除报告。
>
...
- 把地图已有的 **106 议题 / 244 论点 / 39 主张簇** 转成树的初始节点，解决"空树"问题（2026-09-14 修正口径：数据已从初版 15/23/5 扩容）
- **验收**：新建的树非空、有真实议题与论点、可回溯知乎原问题
- **完成标志**：不再有"新树没内容"

### 卡 3-4 · 地图视觉增强（凸包 / 连线分层 / 标签密度）✅

> **2026-09-14 新增（D8 拍板：demo 前必须做）**。依据 `controversy-map-VISUAL-REFERENCES.md` 的 P0 三条，**只动渲染层**，数据层 `controversyMap.ts` / `map.json` 一字未改：
>
> - **A1 凸包分组底色**（`61d4080`）：39 个主张簇各一块低透明度包络垫在最底层；分组依据 = bridge 边（**数据事实**），包络形状才是布局产物，图例须分开讲；采样成"圆集凸包"免写退化特例；悬停分档（含高亮成员浮起、其余退背景）；聚焦视图不渲染
> - **A2 连线透明度分层 + 缝合线弧线化**（`8dbb9ec`）：bridge 0.75 / rebuts 0.6 / member 0.25 / contains 0.2；177 条缝合线改二次贝塞尔弧线，弯向按两端 id 字典序固定，同一条边不左右翻
> - **A3 标签密度自适应**（`8dbb9ec`）：语义缩放三档 k≥1.2（near）/ <0.6（far）/ 其余（mid）；跨档才 setState；聚焦视图强制按 mid；「显示全部标签」按钮降级为手动覆盖
>
> - **红线对照**：凸包分组口径由测试锁定来自 bridge 边，不是几何邻近（不伪造数据）；106 议题中 31 个未被任何主张缝合、不落包络，属真实信息，图例如实呈现（不把 hull 当归属事实）；透明度分层不新增任何数据字段
> - **验收**：全量 223/223 测试通过（新增 11 项：凸包 5 + 连线分层 3 + 标签密度 3）、`tsc -b` 0 错误、构建 466KB / gzip 140KB；PRD 升 v0.2、PLAN v0.1 建立
> - **完成标志**：`?view=map` 骨架视图里"哪些议题是一伙的"与"哪些主张在跨议题缝合"一眼可辨，且无需图例教学
> - **后续**：P1（分层边捆绑 / 聚焦透镜 / 密度等值线）与 P2（小倍数 / 圆周布局）不并入本卡，另立卡；聚焦透镜属交互语义变更，动工前须过 PRD §4 层拍板
