...
> 基线：`event-replay-PRD.md` v0.6（沉浸式历史角色扮演 · 界面壳体对齐交互原型）、`IMPLEMENTATION-PATH.md` 阶段 0/4；2026-09-14 仓库状态。文件名沿用历史名称 `event-replay-*`，产品文案统一使用"事件推演"。
...
- **用途变更（v0.6，2026-09-14）**：该原型转为**视觉与壳体基线，不作语义基线**。前端沿用它的版式（顶栏 + 三栏 + 时间轴圆点 + 内缩分割线卡片 + 终局遮罩对照表）与同一套配色变量；**内容按 v0.5 语义重填**。原型里的旧语义元素明确**不实现**：`%` 占比与「多数人」列、`↺ 从这里重走`、「分歧点沉淀到辩论树」、预设四结局；折叠式「真实历史中当事人做了什么」改写为「只在终局对照里、且玩家主动揭示后才进 DOM」。**仍禁止为了"两版对齐"而回改静态版**。
- 原始声明（v0.5，仍有效的那一半）：原型形态（观察者 + 剧透保护 + 三列对照的**旧语义**）与本版根本不同，**不得把原型 HTML 当作本模块的演示物**——它保留着已砍掉的开辩论入口，演示它会误导评审。离线兜底用**录屏**或本地缓存生成结果。
...
2. **版式基线（v0.6）**：壳体照 `prototypes/event-replay-prototype.html` —— 顶栏（`争鸣` logo + `事件推演 · <事件名>` crumb + 模块按钮组）、三栏（左：事件卡 / 推演路径时间轴 / 图层说明；中：舞台；右：Host 追问 / 我的路径 chips / 本局数据 / 代价账本），配色取原型同一套变量（`--er-blue #056de8`、`--er-ink #121212`、`--er-sub #6b7484`、`--er-line #e5e8ee`、`--er-bg #f6f7f9`、`--er-real #5a6474`、`--er-warn #c2571f`），卡片靠内缩分割线区隔、终局用遮罩对照表。**只借壳不借语义**：`%` 占比、多数人列、从这里重走、开辩论入口、预设结局一律不实现。
3. 实现事件库选择、场景头（含改编声明）、**角色位选择卡**（stake / canDo / resources / 初始关系概览，不显示任何原作信息）、幕时间轴、动作卡（代价提示 + 隐含假设折叠）、生成中/失败态、账本与关系面板、自动收束后的终局遮罩（结局叙述 + 「幕 / 你的选择」对照表，无任何优劣评价），以及表内「历史上实际发生了什么」揭示入口（`--real` 深灰底「史实」标签，只允许出现在终局对照的第三列；玩家路径全程标「架空推演」）。窄屏允许横向滚动，不允许内容重叠。
4. 原作揭示用可访问的展开按钮；**展开前 DOM 中不得渲染 canon 文本/溯源，连「史实」二字都不得出现**（用 `--real`/`--er-tag-real` 只在第三列渲染）。同路人对比（F7）只在结束后出现，标注为演示数据。
5. 新建 `web/tests/eventReplay.test.tsx`，以角色/可访问名称测试行为（选位后进入第 1 幕、选择后禁用其他动作、生成失败可重试、关系反向限制使某动作不可选、结局后才能展开原作）。不复制原型 DOM 桩选择器。
6. **版式核对（dev-only）**：`web/visual-check.html` + `web/src/visual-check.tsx` 是脚本化 mock 的版式夹具（`?auto=play|ending`），供无 Host 时在真浏览器核对三栏与终局遮罩；**不进构建产物**（已确认 `vite build` 的 dist 不含它），内容不得对外当作模型输出。
...
- **版式基线（v0.6 新增）**：三栏壳体、配色变量与原型同源（见 §4 阶段 C 第 2 条）；`er-tag-real`（史实标签）在推演期间**必须为零**——UI 测试断言 `document.body` 不含「史实」二字；终局对照表在揭示前只有「幕 / 你的选择」两列，揭示后才追加第三列。版式核对用 `web/visual-check.html?auto=play|ending` 截图比对（人工步骤，非 CI）。
...
| 借壳导致语义回潮 | 版式照原型后，容易被顺手把原型的旧语义（占比、多数人、重走、开辩论）一起搬回来。缓解：PRD §8 列出逐项处置表；代码里以注释写明"只借壳不借语义"；新增任务卡验收要检查这四项缺席。 |
| 版式夹具被当成模型输出 | `web/visual-check.html` 用 mock 数据，若被当作真实推演截图对外展示，等于伪造效果。缓解：夹具文件头写明 dev-only；确认 `vite build` 不含它；对外截图一律走 `?view=event` 真实链路。 |
