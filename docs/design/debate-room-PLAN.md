...
- ~~不把模型对手包装成"真人实时对局"~~ → **v0.7 收窄为**：需要真人对局时走真实房间通道（D8）；Bot 兜底路径仍**不得伪造"真人在线"提示**（即不把 Bot 包装成真人）
- ~~真正的多人实时房间仍缺身份、候选池、房间生命周期和消息传输契约，须单独决策后再承诺。~~ → **v0.7 作废**：已立为决策点 **D8** 并拍板纳入本期（见 §6）。
...
- **D8 真实多人房间（已拍板 2026-09-14：做）**：用户要求「双方可以真的进来把一场辩论打完」。拍板内容——
  - **传输**：WebSocket（`zhengming-server` 内，`ws://<host>/ws/room`），与 Host HTTP 同进程同端口。
  - **权威状态**：**服务端权威**。客户端只发动作（`pickSide`/`submitBrief`/`submitOpening`/`ask`/`answer`/`react`/`freeSpeak`/`submitClosing`/`leave`），服务端校验后广播新状态快照；客户端不做乐观状态推进（避免双方状态分叉）。
  - **房间生命周期**：`create`（选议题+选边）→ `join`（守另一边的真人可加入；也可直接 `create` 后开 Bot 兜底）→ `playing` → `settled` → 报告落 `rooms/<id>.json`（D5）→ 到期清理（闲置 2 小时）。
  - **退出与离席**：一方 `leave` 或断线超时（90s）→ 房间标记 `abandoned`，离席方 MP -5（PRD §7），另一方收到「对方已离席」并生成「未完成局」报告；断线重连（同 `seatToken`）可恢复席位。
  - **身份**：无账号体系，用**席位令牌**（`seatToken`，服务端签发、存 localStorage）。同一房间两个席位：`seat-pro` / `seat-con`（按选边定，选边即占位）。**这是演示级身份，不是登录系统**——文档与 UI 都不宣称有账号。
  - **已知限制（写进 README，不隐瞒）**：同一浏览器多标签页共用 localStorage，演示真人对局需**一个正常窗口 + 一个隐身窗口**，或两台设备连同一局域网地址。
- **D9 Host 上游改 StepFun（已拍板 2026-09-14）**：原契约写死 DeepSeek（`api.deepseek.com` + `deepseek-chat`）。用户指定改用 **StepFun 阶跃星辰**：`https://api.stepfun.com/v1/chat/completions`、`model: "step-3.7-flash"`、OpenAI 兼容、支持 `response_format:{type:"json_object"}`。环境变量名 `STEPFUN_API_KEY`（原 `DEEPSEEK_API_KEY` 作废）。`host-contract.md` §0.3 参数基线与 §0.2 密钥段落须同步改。
- **真人撮合（已拍板 2026-09-13：真人优先、Bot 兜底 → v0.7 升级）**：原方案「候选池 = 守另一边的选边用户」，实际落地为 **D8 的显式房间**：真人对手通过房间通道进入（不再依赖候选池排队撮合）；候选池空时仍由 Bot 持预设论点开局，**不伪造真人在线**。排队撮合（自动为陌生人配对）**仍不做**——本期是「邀请式/双开式」真人对局。
...
**v0.7 追加完成定义**：① `zhengming-server/` 落地并可通过 `POST /api/host/:capability` 真实调用 StepFun；② **两个浏览器上下文可进入同一房间、按五阶段走完一场对局**，服务端状态全程权威；③ 对局报告落 `rooms/<id>.json` 且与终局卡一致；④ `web/` 顶栏第三个标签可进入辩论间（`?view=room`）；⑤ 无 key 时 Host 优雅降级（回退启发式）并在 UI 明示「模拟」；⑥ 真实多人路径在局域网/双开场景下人工验收通过。
